...
def user_aided(state, request: gr.Request):
    ip = get_ip(request)
    logger.info(f"regenerate. ip: {ip}")
    state.not_user_input = True
    return (state, state.to_gradio_chatbot(), "") + (disable_btn,) * 11

def add_suggestion(state, model_selector, sug_idx, request: gr.Request):
    ip = get_ip(request)
    logger.info(f"add_suggestion. ip: {ip}. Suggention: {sug_idx}")
    if state is None:
        state = State(model_selector)
    text = f"Based on suggestion {sug_idx}, what is this symptom and what clinic should I go ?"
...
def add_text(state, model_selector, text, request: gr.Request):
    ...
    if(state.not_user_input == True):
        text = " Based on suggestion 1, what is this symptom and what clinic should I go ?"
        #print(text)
    else: 
...
"""
    ...
    for i, btn in enumerate([suggestion1_btn, suggestion2_btn, suggestion3_btn, suggestion4_btn], start=1):
        btn.click(
            add_suggestion,
            [state, model_selector, gr.Number(i)],
