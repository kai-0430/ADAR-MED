...
def regenerate(state, request: gr.Request):
    ...
    if not state.regen_support:
        ...
        return (state, state.to_gradio_chatbot(), "", None) + (no_change_btn,) * 11
    state.conv.update_last_message(None)
    return (state, state.to_gradio_chatbot(), "") + (disable_btn,) * 11

# def user_aided(state, request: gr.Request):
#     ip = get_ip(request)
#     logger.info(f"regenerate. ip: {ip}")
#     state.skip_next = True
#     return (state, state.to_gradio_chatbot(), "", None) + (no_change_btn,) * 11   

def user_aided(state, request: gr.Request, suggestion_number: int):
    ip = get_ip(request)
    logger.info(f"user_aided. ip: {ip}")
    # state.skip_next = True
    ...
    text = f"Based on Suggestion {suggestion_number}, What should I do? What is the symptom?"
...
def bot_response(
    ...
    if state.skip_next:
        ...
        yield (state, state.to_gradio_chatbot()) + (no_change_btn,) * 11
        return
    ...
    yield (state, state.to_gradio_chatbot()) + (disable_btn,) * 11
    ...
    try:
        ...
        output = data["text"].strip()
        conv.update_last_message(output)
        yield (state, state.to_gradio_chatbot()) + (enable_btn,) * 11
...
#     )
    promotion = """
        ### Links
        [Proposal](https://www.hackster.io/contests/amd2023/hardware_applications/16954) | [Report](https://www.hackster.io/contests/amd2023/hardware_applications/16954) | [GitHub](https://www.hackster.io/contests/amd2023/hardware_applications/16954)
    """
    
    notice_markdown = f"""
# 🏔️ AI-Driven Health: Your Intelligent Medical Assistance
{promotion}
"""
    ...
    with gr.Row() as button_row:
        suggestion1_btn = gr.Button(value="Suggestion 1", interactive=False)
        suggestion2_btn = gr.Button(value="Suggestion 2", interactive=False)
        suggestion3_btn = gr.Button(value="Suggestion 3", interactive=False)
        suggestion4_btn = gr.Button(value="Suggestion 4", interactive=False)
        
            # Action buttons including suggestion buttons
    with gr.Row() as button_row:
        symptom_btn = gr.Button(value="What Symptom may I have ?", interactive=False)
        clinic_btn = gr.Button(value="What clinic should I go ?", interactive=False)
    ...
    btn_list = [regenerate_btn, clear_btn, suggestion1_btn, suggestion2_btn, suggestion3_btn, suggestion4_btn, symptom_btn, clinic_btn]
    # upvote_btn.click(
    ...
    suggestion1_btn.click(clear_history, None, [state, chatbot, textbox] + btn_list)
    suggestion2_btn.click(clear_history, None, [state, chatbot, textbox] + btn_list)
    suggestion3_btn.click(clear_history, None, [state, chatbot, textbox] + btn_list)
    suggestion4_btn.click(clear_history, None, [state, chatbot, textbox] + btn_list)
    symptom_btn.click(clear_history, None, [state, chatbot, textbox] + btn_list)
    clinic_btn.click(clear_history, None, [state, chatbot, textbox] + btn_list)
    ...
    # 处理 suggestion1_btn 到 suggestion4_btn 的点击事件
    for i, btn in enumerate([suggestion1_btn, suggestion2_btn, suggestion3_btn, suggestion4_btn], start=1):
        btn.click(user_aided, state, [state, chatbot, textbox] + btn_list).then(
            bot_response,
            [state, temperature, top_p, max_output_tokens],
            [state, chatbot] + btn_list,
        )
    
#     # 处理 symptom_btn 的点击事件
#     symptom_btn.click(
#         lambda: set_input_text("What Symptom may I have?"),
#         state,
#         [state, chatbot, textbox] + btn_list
#     ).then(
#         bot_response,
...
#     # 处理 clinic_btn 的点击事件
#     clinic_btn.click(
#         lambda: set_input_text("What clinic should I go?"),
#         state,
#         [state, chatbot, textbox] + btn_list
#     ).then(
#         bot_response,
